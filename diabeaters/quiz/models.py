...
class Quiz(models.Model):
    ...
    def edit_form(self):
        class EditForm(forms.Form):
            ...
            alt_text = "<a href=\"" + reverse(
                "edit-quiz",
                args=[self.id]) + "\">manage questions/answers</a>"
        return EditForm()
...
class Question(models.Model):
    ...
    def user_responses(self, user):
        submission = Submission.objects.filter(
            user=user, quiz=self.quiz).order_by("-submitted")[0]
...
class Submission(models.Model):
    ...
    def __unicode__(self):
        return "quiz %d submission by %s at %s" % (
            self.quiz.id, unicode(self.user), self.submitted)
...
class Response(models.Model):
    ...
    def __unicode__(self):
        return "response to %s [%s]" % (
            unicode(self.question), unicode(self.submission))
