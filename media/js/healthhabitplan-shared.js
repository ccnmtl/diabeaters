...
function togglePopup() {
    ...
    popup.css('top', jQuery(this).offset().top - fridgeOffset.top -
              popup.height() + windowOffsetTop + 20);
    popup.css('left', jQuery(this).offset().left - fridgeOffset.left -
              popup.width() + windowOffsetLeft - 20);
    jQuery(popup).show();
}

jQuery(document).ready(function() {
    jQuery('.magnet').click(togglePopup);
    jQuery('.magnet-popup-close').click(function() {
        jQuery(this).parent('.magnet-popup').hide();
    });
});
